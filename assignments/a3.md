...
about the boundary bucket.
* MapReduce part updated: The Gutenberg text dataset is silently
updating in the background, resulting in inconsistent test results. If you
...
* ~~a `download_gutenberg.sh` bash script to download the text files as the input;~~ (Project Gutenberg is updating the text of its eBooks, causing inconsistent test results; therefore, we will use a snapshot of the text datasets from 98 eBooks downloaded in Feburary 2024 for testing.)
